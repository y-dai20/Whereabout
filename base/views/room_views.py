...
class ManageRoomDisplayView(ManageRoomBaseView, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not f.is_empty(tags_str):
            tags = tags_str.split(',')
            self.room.tag1 = f.get_tag(f.get_list_item(tags, 0), self.request.user)
            self.room.tag2 = f.get_tag(f.get_list_item(tags, 1), self.request.user)
            self.room.tag3 = f.get_tag(f.get_list_item(tags, 2), self.request.user)
            self.room.tag4 = f.get_tag(f.get_list_item(tags, 3), self.request.user)
            self.room.tag5 = f.get_tag(f.get_list_item(tags, 4), self.request.user)
            self.room.tag6 = f.get_tag(f.get_list_item(tags, 5), self.request.user)
            self.room.tag7 = f.get_tag(f.get_list_item(tags, 6), self.request.user)
            self.room.tag8 = f.get_tag(f.get_list_item(tags, 7), self.request.user)
            self.room.tag9 = f.get_tag(f.get_list_item(tags, 8), self.request.user)
            self.room.tag10 = f.get_tag(f.get_list_item(tags, 9), self.request.user)
